...
class ModifiedCheckpointHook(CheckpointHook):
    def before_train(self, runner) -> None:
        ...
        self.file_backend = fileio.get_file_backend(self.out_dir, backend_args=self.backend_args)
        if self.out_dir != runner._log_dir:
            basename = os.path.basename(runner.work_dir.rstrip(os.path.sep))
            self.out_dir = fileio.join_path(self.out_dir, basename)  # type: ignore  # noqa: E501
    ...
    def _save_best_checkpoint(self, runner, metrics) -> None:
        ...
        for key_indicator, rule in zip(self.key_indicators, self.rules):
            ...
            if best_ckpt_path and fileio.isfile(best_ckpt_path) and dist.is_main_process():
                fileio.remove(best_ckpt_path)
    ...
    @dist.master_only  # need this decorator to avoid multiple processes saving the same checkpoint
    def _publish_model(self, runner, ckpt_path: str) -> None:
        ...
        if removed_keys:
            print_log(
                ...
                logger='current')
        final_path = os.path.splitext(ckpt_path)[0] + '.pth'  # overwrite the original checkpoint
