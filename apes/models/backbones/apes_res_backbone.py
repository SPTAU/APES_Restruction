...
from ..utils.layers import (Embedding, GlobalDownSample, LocalDownSample,
                            N2PAttention, UpSample)


@MODELS.register_module()
class APESResBackbone(BaseModule):
    def __init__(self, which_ds, init_cfg=None):
        super(APESResBackbone, self).__init__(init_cfg)
        ...
        self.conv2 = nn.Sequential(nn.Conv1d(40, 128, 1, bias=False), nn.BatchNorm1d(128), nn.LeakyReLU(0.2))
    ...
    def forward(self, x, shape_class):
        ...
        shape_class = self.conv2(shape_class)  # (B, 40, 1) -> (B, 128, 1)
        x, _ = pack([x, shape_class], 'B *')  # (B, 2048) -> (B, 2176)
        x = repeat(x, 'B C -> B C N', N=2048)  # (B, 2176) -> (B, 2176, 2048)
        x, _ = pack([x, x1], 'B * N')  # (B, 2176, 2048) -> (B, 2304, 2048)
        return x
